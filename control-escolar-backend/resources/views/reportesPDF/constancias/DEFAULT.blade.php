...
<head>
    ...
    <meta charset="UTF-8">
    <link rel="stylesheet" type="text/css" href="{{public_path().'/css/constancias/BCS.css'}}" media="all">
</head>
<body>
    ...
    <div class="row texto">
        ...
        semestral del {{$alumno->inicioPeriodo}} al {{$alumno->finPeriodo}}.
    </div>
    <div class="row texto">
        <br>
        {{$descripcion}}
    ...
    <br>
    <div class="row texto">
        Por lo que a petición del interesado y para los fines legales que haya lugar, se extiende la presente con {{$alumno->promedioCreditos['creditos']}} créditos cursados de un total de {{$alumno->carrera->total_creditos}},
        con un promedio general de {{number_format($alumno->promedioCreditos['promedio'], 1)}} ({{$alumno->promedioCreditos['promedioConLetra']}}) en {{$alumno->plantel->ciudad}},
    ...
    <div class="row">
        ...
        @if($alumno->plantel->personal != null)
            <p>{{$alumno->plantel->personal->nombre_director}} <br>
                ...
                @else
                    Responsable del Centro {{(!str_contains(strtolower($alumno->plantel->nombre_final), 'emsad')) ? 'EMSaD' : ''}}
                @endif
