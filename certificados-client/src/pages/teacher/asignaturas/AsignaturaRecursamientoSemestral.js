...
export default ({ match }) => {
  ...
  const getData = async (docenteAsignaturaId) => {
    setLoading(true);
    const docenteAsignaturaApi = await getSubjectRecursamientoSemestral(docenteAsignaturaId);
  ...
  return (
    <>
      <Loading loading={loading}>
        ...
        <TitleBar>Captura De Calificaciones Recursamiento Semestral</TitleBar>
        <CapturaCalificacionesRecursamiento docenteAsignatura={docenteAsignatura} onSave={() => getData(match.params.docenteAsignaturaId)} tipoRecursamiento={1} />
      </Loading>
    </>
  );
};
