...
import { Loading, PrimaryButton, SearchSelect, Subtitle } from "../../shared/components";
...
export default function StudentCertificate({ curp, setCurp, editable }) {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(true);
  const [studentData, setStudentData] = useState({});
  
  const [toggleEdit, setToggleEdit] = useState(true);
  const [diciplinaryCompetence, setDiciplinaryCompetence] = useState([]);
  ...
  useEffect(() => {
    const getStudentData = async () => {
      ...
      const competence= await getDisplinaryCompetence();
    
      setDiciplinaryCompetence( competence )
      setLoading(false);
  ...
  const editStudent = async (values) => {
    setLoading(true);
    const response = await CertificateService.editStudent(values, values.isPortability=0, values.isAbrogado=0);
    setLoading(false);
    if (!response.success) return;
    await reloadStudentData();
        ...
        >
         ...
         {/* <Row {...rowProps}> 
          ...
          <Row {...rowProps}>
            ...
            <Col {...colProps}>
              <Form.Item label="Competencia disciplinar extendida:" name="disciplinaryCompetence" rules={validations.default}>
                <SearchSelect dataset={diciplinaryCompetence} />
              </Form.Item>
            </Col>
