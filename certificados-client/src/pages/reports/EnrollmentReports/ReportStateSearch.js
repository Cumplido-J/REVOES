...
import { Loading,  SearchSelect, ButtonCustom, ButtonExcelReportEnrollmentSchool, ButtonExcelReportEnrollmentSchoolCareer } from "../../../shared/components";
...
import ReportSchoolCareerTable from "./ReportSchoolCareerTable";
...
export default function ReportStateSearch({ onSubmit }) {
  ...
  const onClickBuscar = async () => {
    ...
    let encontro=false;
    const response = await ReportsSchoolEnrollmentService.getStateEnrollment(schoolType, cicloId, enrollId);
    if (!response)
    {
  ...
  const onClickEstado = async (idEntidad) => {
    ...
    setStateSchoolCareerReport([]);
    let cicloId=form.getFieldsValue().cicloId;
    let matricula=form.getFieldsValue().enrollId;
    let tipoPlantel=form.getFieldsValue().schoolType;

    //guardamos si encontro datos
    let encontro=false;
    const response = await ReportsSchoolEnrollmentService.getStateSchoolEnrollment(tipoPlantel,cicloId,matricula,idEntidad)
    if (!response)
    {
      Alerts.warning("Éxito", "No se encontraron datos");
      return;
    ...
    const onClickPlantel = async (idPlantel) => {
      ...
      //guardamos si encontro datos
      let encontro=false;
      const response = await ReportsSchoolEnrollmentService.getStateSchoolCareerEnrollment(cicloId, enrollId, idPlantel)
      if (!response)
      {
        Alerts.warning("Éxito", "No se encontraron datos");
        return;
  ...
  return (
    <>
      ...
      <Loading loading={loading} >
        ...
        <br/>
        <Col {...colProps}>
            <ButtonExcelReportEnrollmentSchool dataset={stateSchoolReport} filename={`ReporteMatriculaPlantel_${f}_${h}`} loading={loading} />
        </Col>
        ...
        <br/>
        <Col {...colProps}>
            <ButtonExcelReportEnrollmentSchoolCareer dataset={stateSchoolCareerReport} filename={`ReporteMatriculaCarrera_${f}_${h}`} loading={loading} />
        </Col>
        <ReportSchoolCareerTable dataset={stateSchoolCareerReport}/>
      </Loading>
    </>
  );
}