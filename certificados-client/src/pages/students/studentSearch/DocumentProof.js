...
const DocumentProof = ({ student, icon = true }) => {
  ...
  const handleOnClickDocumentProof = async (data) => {
    setLoading(true);
    const pdfResponse = await StudentService.studentDocumentProof(student.usuario_id, data);
  ...
  const handleShowModal = async () => {
    const respondeState = await CatalogService.getStateId(student.plantel_id);
    ...
    //form.setFieldsValue({ descripcion: "Y con un periodo vacacional del 11 al 22 de abril del 2022.", fotografia: false});
    setShowModal(true);
  }
