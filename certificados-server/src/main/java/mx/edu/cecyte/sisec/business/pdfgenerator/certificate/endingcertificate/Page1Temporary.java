...
public class Page1Temporary extends EndingCertificateShared {
    private final EndingPdfData pdfData;

    public Page1Temporary(PdfResources pdfResources, EndingPdfData pdfData, Document document) {
    ...
    public void generate() {
        ...
        getModuleParagraph();
        getStampParagraph();
        //document.getPdfDocument().addEventHandler(PdfDocumentEvent.INSERT_PAGE, );
    ...
    private void getStudentParagraph() {
        ...
        celda3.add(studentMatriculontainer).setBorder(Border.NO_BORDER).setBorderBottom(new SolidBorder(colorLine, 0.2f)).setPadding(PdfFunctions.cmToPoints(0f));
        ...
        if (pdfData.isCecyte()) {
            ...
            //else schoolNameString = nameComplete1 + "Plantel " + pdfData.getSchoolName();
        } else if (pdfData.isEmsad()) {
            schoolNameString = nameComplete2 + pdfData.getSchoolName();
            /*if (schoolNameString.contains("Plantel")) schoolNameString = schoolNameString;
            else schoolNameString = nameComplete2 + "Plantel " + pdfData.getSchoolName();*/
        }

        Text schoolNameStringText = newTextMontserrat(schoolNameString, 8f);
        Paragraph schoolNameContainer = PdfFunctions.createParagraph().add(schoolNameStringText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        ...
        Paragraph claveNumberContainer = PdfFunctions.createParagraph().add(claveNumberText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        Paragraph validationNameContainer = PdfFunctions.createParagraph().add(validationNameText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        ...
        Text planNameText = newTextMontserrat(planNameString, 8f);
        Paragraph planNameContainer = PdfFunctions.createParagraph().add(planNameText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);

        Cell planNameCell = PdfFunctions.createCell().add(planNameContainer);

        Table planNameTable = new Table(new float[]{PdfFunctions.cmToPoints(18.19f)});
        ...
        Table planDescriptionTable = new Table(new float[]{PdfFunctions.cmToPoints(17.59f)});
        planDescriptionTable.setWidth(PdfFunctions.cmToPoints(17.59f)).setMarginLeft(PdfFunctions.cmToPoints(1f));
        ...
        if (pdfData.isPortabilty())
            ...
            claveNameString = pdfData.getCareerKey() != null ? pdfData.getCareerKey() : "*******************";
        //else claveNameString = "*******************";
        ...
        Paragraph claveNameContainer = PdfFunctions.createParagraph().add(claveNameText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        Paragraph periodoContainer = PdfFunctions.createParagraph().add(periodoText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        ...
        Paragraph creditContainer = PdfFunctions.createParagraph().add(creditText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        Paragraph totalContainer = PdfFunctions.createParagraph().add(totalText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
        Paragraph scoreContainer = PdfFunctions.createParagraph().add(scoreText).setTextAlignment(TextAlignment.CENTER).setPadding(0.5f);
    ...
    private void getModuleParagraph() {
        String moduleString = "\nMódulos acreditados de formación técnica:";
        if (pdfData.isEmsad())
            moduleString = "\nAsignaturas acreditadas de formación elemental para el trabajo:";
        ...
        Paragraph moduleContainer = PdfFunctions.createParagraph().add(moduleText).setTextAlignment(TextAlignment.JUSTIFIED).setMarginLeft(PdfFunctions.cmToPoints(1f));
        ...
        modulesTable.setWidth(PdfFunctions.cmToPoints(18.19f)).setMarginLeft(PdfFunctions.cmToPoints(0.7f));
        ...
        for (int i = 0; i < pdfData.getModules().size(); i++) {
            ...
            Text numberText = newTextMontserrat(number, 7f);
            Text nameText = newTextMontserrat(module.getName(), 7f);
            Text scoreText = newTextMontserrat(module.getScore(), 7f);
            Text hoursText = newTextMontserrat(module.getHours(), 7f);
            Text creditsText = newTextMontserrat(module.getCredits(), 7f);
    ...
    private void getStampParagraph() {
        ...
        Paragraph stampContainer = getStampContainerTempory(pdfData).setTextAlignment(TextAlignment.JUSTIFIED).setFontSize(7f);
        Paragraph folioContainer = getFolioContainerTemporary(pdfData.getFolioNumber());
        Paragraph printContainer = PdfFunctions.createParagraph().add(printText).setTextAlignment(TextAlignment.JUSTIFIED);
